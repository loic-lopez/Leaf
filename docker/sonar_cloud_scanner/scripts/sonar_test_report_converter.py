...
class SonarTestExecutionsXmlSerializer:
                ...
                if regexp.search(fileName) is not None:
                    ...
                    for key, value in executionDict.items():
                        if key not in sonarTestExcutions:
                            sonarTestExcutions[key] = SonarTestExecution(key)

                        sonarTestExcutions[key].add_test_cases(value.test_cases)

        return list(sonarTestExcutions.values())
                ...
                for testCase in testCases:
                    ...
                    if testName in test_name_to_source_name:
                        ...
                        if test_file_name not in sonarTestExecutions:
                            sonarTestExecutions[test_file_name] = SonarTestExecution(test_file_name)
            ...
            for fileName in fileList:
                if regexp.search(fileName) is not None:
                    ...
                    with open(file_path, 'r') as src_file:
                        file_lines = src_file.read()
                        for match in GTEST_RE.finditer(file_lines):
                            test_name_to_source_name['{0}.{1}'.format(match.group(1).strip(), match.group(2).strip())] = file_path
        ...
        try:
            ...
            for testCase in testCases:
                if testCase.get('file') not in sonarTestExecutions:
                    sonarTestExecutions[testCase.get('file')] = SonarTestExecution(testCase.get('file'))
...
if __name__ == '__main__':
    ...
    if args.report_type == 'junit':
        ...
        test_cases = GoogleTestReportParser.parse(args.search_folder, args.report_pattern,
                                                  args.gtest_src_folder, args.gtest_src_pattern)

    with open(os.path.realpath(args.output), 'wb') as file:
        file.write(SonarTestExecutionsXmlSerializer.serialize(test_cases))